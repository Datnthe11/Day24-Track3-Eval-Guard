...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            joined_group = " ".join(current_chunk_sentences)
            chunks.append(Chunk(text=joined_group, metadata={**metadata, "strategy": "semantic", "chunk_index": len(chunks)}))
    ...
    for section in sections:
        if re.match(r'^#{1,3}\s+', section):
            if current_content.strip() or current_header:
                ...
                if combined:
                    chunks.append(Chunk(text=combined, metadata={**metadata, "section": current_header, "strategy": "structure"}))
