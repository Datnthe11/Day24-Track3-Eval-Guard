...
def cohen_kappa(judge_labels: list[int], human_labels: list[int]) -> float:
    ...
    p_e = (judge_labels.count(1)/n * human_labels.count(1)/n +
           judge_labels.count(0)/n * human_labels.count(0)/n)
    κ = (p_o - p_e) / (1 - p_e) if p_e != 1 else 1.0
    return κ
...
if __name__ == "__main__":
    import os
    # --- Load human labels and ground truth ---
    ...
    print(f"Human labels loaded: {len(human_data)} questions")
    print("Running swap-and-average judge on 10 questions...")
    ...
    for item in human_data:
        ...
        a_ground = ground_truths[q_id]
        
        # A_A is model_answer, A_B is ground_truth
        result = swap_and_average(q, a_model, a_ground)
        judge_results.append(result)
        
        # If model_answer (A) wins or ties with ground truth, we consider it a good answer (1)
